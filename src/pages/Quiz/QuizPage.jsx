...
function shuffle(array) {
  ...
  while (arrayLength--) {
    randomNumber = Math.floor(
      crypto.getRandomValues(cryptoArray) / Math.pow(10, 9)
    );
...
function QuizPage() {
  ...
  const handleAnswerButtonClick = (answer) => {
    if (userAnswerArray[currentQuestion] === undefined) {
      if (
        dummyQuestions[currentQuestion].correct === answer.target.textContent
      ) {
    ...
    if (currentQuestion === dummyQuestions.length - 1) setProgressValue(100);
  };
